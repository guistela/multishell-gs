...
## 🤖 Recursos para Agentes de IA & Pair Programming

O Multishell foi desenhado para maximizar a produtividade e segurança ao parear com assistentes de IA (Claude Code, Gemini CLI, Cursor, Antigravity, Aider, Devin):

- **Cockpit Unificado & Minimalista**: Interface moderna com abas integradas (`Terminais`, `Kanban`, `Métricas`, `Snapshots`, `Segurança`), expandindo a área útil vertical e eliminando redundâncias visuais.
- **Kanban Ágil Sincronizado via `gh` CLI**: Sincronização nativa de issues e tarefas utilizando as credenciais isoladas da sessão ativa do terminal (`gh issue list` com isolamento por espaço), além de suporte a Azure Boards e Jira.
- **MCP Hub Isolado por Espaço**: Conectores [Model Context Protocol](https://modelcontextprotocol.io/) configurados e isolados por perfil (`mcp.json`).
- **Git Worktree Sandbox**: Criação instantânea de branches e diretórios paralelos para agentes executarem tarefas sem prender o diretório principal de trabalho.
- **Agent-to-Agent Bus**: Barramento PUB/SUB desacoplado para comunicação e orquestração de enxames de terminais e agentes.
- **Guardrail Sentinela**: Bloqueio ativo de comandos destrutivos acidentais (`rm -rf /`, `fork bomb`, formatação de disco, SQL destrutivo).
- **Time Machine & Snapshots**: Captura diferencial do estado do código antes de tarefas automatizadas para auditoria e rollback imediato.
- **Live Token & Cost Meter**: Telemetria em tempo real de tokens consumidos e custos acumulados por sessão e modelo.
- **Atuação Contratual Multiagente**: Definição de papéis especialistas (Arquiteto, Implementador, QA, SecOps), validação de entregáveis e gates de aprovação.
- **Conselho Multi-Modelo & Handoffs**: Protocolo estruturado de passagem de bastão contínua entre harnesses diferentes (Claude ➔ Gemini ➔ Codex) e arbitragem de divergências.
- **Memória Semântica & Truncador Inteligente**: RAG local em histórico de comandos e compressão de logs com sentinela de tokens.

👉 Veja o detalhamento completo em [Guia de Contratos Multiagente](docs/AGENT-CONTRACTS-GUIDE.md) e no [Roadmap de Agentes de IA](docs/ROADMAP-AGENT-FEATURES.md).
