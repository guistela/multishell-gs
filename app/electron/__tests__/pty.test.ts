...
describe("PtyManager — anexos e ring buffer (fase 8)", () => {
  it("spawn é idempotente: segunda chamada anexa e devolve replay", async () => {
    const pty = new PtyManager();
    managers.push(pty);
    const a = fakeWindow();
  ...
  it("ring buffer mantém só os últimos 256 KB", () => {
    const pty = new PtyManager();
    managers.push(pty);
    const a = fakeWindow();
  ...
  it("broadcast só para anexados e vivos; destruídos são removidos", async () => {
    const pty = new PtyManager();
    managers.push(pty);
    const a = fakeWindow();
    const b = fakeWindow();
