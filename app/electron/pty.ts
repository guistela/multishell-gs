...
interface PtySession {
  ...
  buffer: RingBuffer;
  /** Métricas reais da sessão. Só bytes: token não dá para saber daqui. */
  bytesIn: number;
  bytesOut: number;
  startedAt: string;
}
...
export interface SessionStats {
  session_id: string;
  /** Raiz da árvore de processos da sessão: serve para medir memória. */
  pid: number;
  bytes_in: number;
  bytes_out: number;
  started_at: string;
...
export class PtyManager {
  ...
  write(sessionId: string, data: Uint8Array | number[] | string): void {
    ...
    s.bytesIn += Buffer.byteLength(text, "utf8");
    s.proc.write(text);
  }
  ...
  kill(sessionId: string): void {
    ...
    this.sessions.delete(sessionId);
    s.buffer.clear();
    // Zera junto com o buffer: a sessão morreu, a métrica dela não sobrevive.
    s.bytesIn = 0;
    s.bytesOut = 0;
    try {
      s.proc.kill();
  ...
  /** Métricas da sessão. `null` se ela não existe (ou já foi morta). */
  ...
  allStats(): SessionStats[] {
    return [...this.sessions.keys()].map((id) => this.stats(id)!);
  }
