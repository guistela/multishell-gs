...
const THEMES: Record<string, ITheme> = {
  dark: { background: "#0f1117", foreground: "#e6e6e6", cursor: "#e6e6e6" },
...
export function Terminal({ session, onOpenSettings }: Props) {
  ...
  useEffect(() => {
    ...
    const term = new XTerm({ cursorBlink: true, fontFamily: fontStack(font_family), fontSize: font_size, theme: themeFor(theme) });
    ...
    const unlisteners: Array<() => void> = [
      pty.onOutput(sessionId, (bytes) => { signalFirstOutput(); term.write(bytes); }),
      pty.onExit((exit) => {
    ...
    (async () => {
      ...
      if (result.attached) useAppStore.getState().consumeRestored(sessionId);
      else await resumeIfRestored(sessionId, firstOutput, () => disposed);
  ...
  }, [settings.font_family, settings.font_size, settings.theme]);
