...
export function providerGlyph(p: Provider | undefined): string {
  if (!p) return "";
  // Ícone não-ASCII (emoji) vale como está; nome de símbolo vira a inicial.
  return /^[^\x00-\x7F]/.test(p.icon) ? p.icon : p.name.charAt(0).toUpperCase();
}
    ...
    >
      ...
      {provider && (
        <span className="provider-glyph" title={provider.name}>
          ...
          {session.harness_running && <span className="harness-dot" data-testid="harness-dot" title={t("sidebar.harnessRunning")} />}
        </span>
      )}
      {editing ? (
        <input
      ...
      {session.bypass && <span className="bypass-flag" title={t("sidebar.bypassOn")}>⚡</span>}
    </li>
  );
}
