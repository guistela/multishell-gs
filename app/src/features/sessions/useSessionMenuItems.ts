...
export function useSessionMenuItems({ session, onRename, onOpenSettings }: Props): MenuItem[] {
  ...
  return [
    ...
    {
      id: "session-theme",
      label: t("session:menu.theme"),
      icon: "🎨",
      children: [
        {
          id: "theme-default",
          label: t("session:menu.themeDefault"),
          onClick: () => updateSession(session.id, { theme: undefined }),
        },
        ...THEME_NAMES.map((name) => ({
          id: `theme-${name}`,
          label: `${THEME_LABELS[name] ?? name}${session.theme === name ? " ✓" : ""}`,
          onClick: () => updateSession(session.id, { theme: name }),
        })),
      ],
    },
    { separator: true },
    ...handoffMenuItems(session),
    { separator: true },
    {
      id: "restart",
